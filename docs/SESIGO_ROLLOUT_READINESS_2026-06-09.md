...
| Coordinator rollup server-side | ✅ Complete | `analysis/coordinator_rollup.py::compute_target_actuals` (approved aggregates over project-hierarchy subtree, own/child split, FY quarter overlap, canonical-indicator match); exposed on `CoordinatorTargetSerializer` (actual/own/achievement/variance/status/child_contributions) via viewset context; FE prefers server values (client compute = fallback). 8 tests (`analysis/test_coordinator_rollup.py`). |
| Unified audit log | ✅ Core complete | New `audit` app: `AuditEvent` model + `record_audit_event` helper; admin-only API `/api/audit/events/` (filter by actor/action/object/org/project/date + CSV export) + Django admin view; wired into aggregate create/review/approve/reject/flag/unflag/bulk-approve. 8 tests. **Remaining (cheap):** also call the helper from imports/uploads + project/org admin actions (auth already in `UserActivity`). |
...
| Frontend default-project + assignment UI | ✅ Complete (assignment/target workflows) | Reusable `useDefaultProject` + `NoProjectEmptyState` (2026-06-10) wired into every free-floating-picker assignment/target screen: **aggregates**, **coordinator targets** (`targets/coordinators`), **indicator targets** (`quarterly-targets-section`) — auto-select, current-project context, multi-project guidance, empty state, action-guard, **training isolation** (fixed gap where quarterly targets listed all projects regardless of mode). Project **setup** (`projects/[id]`) is inherently URL-scoped (header = current project), no picker needed. Also wired: **batch-record** import (auto-select, banner, guidance, empty state, mode isolation, import blocked without project) and the coordinator **create/bulk-assign dialogs** now inherit the page project (no cross-project mistakes). Also wired: **batch-record** import (auto-select, banner, guidance, empty state, mode isolation) now with a **role gate** (`canSubmitData` — admin/manager/officer/collector; clients blocked) and **org-scoped** selector (non-admins → own org only, mirroring aggregates). Project list is permission-scoped server-side (`ProjectViewSet`: non-admins see only `assigned_projects`; tested). Remaining (non-write): dashboard project default; interactions keep project *optional by design*. |
...
| R3 | Coordinator totals drift (client rollup) | Low | Med | ✅ Closed | Server-side rollup shipped 2026-06-10 (`analysis/coordinator_rollup.py`); FE consumes authoritative server values |
...
| R6 | Weak forensics (no audit log) | Low | Med | 🟡 Mitigated | Unified `audit.AuditEvent` stream + admin API/CSV export; approval/review/create events recorded 2026-06-10. Extend recording to imports/admin actions to fully close. |
