...
import { renderCompactPieLabel } from "./shared-chart-utils";

export function IndicatorPieWidget({
  metrics,
  subtitle,
  title,
}: {
  ...
  const chartData = useMemo(
    () => buildChartData({ chartType: "indicator-pie-chart", metrics }),
    [metrics],
  ) as Array<{ color: string; label: string; shortLabel: string; value: number }>;
  ...
  return (
    <DashboardPanel eyebrow="Custom widget" subtitle={subtitle} title={title}>
      {chartData.length === 0 ? (
        <ChartEmptyState message="Choose one or more indicators with reported values for this pie chart." />
      ) : (
