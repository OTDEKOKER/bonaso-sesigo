"""Push questionnaire schema v19 live (idempotent; see 0006-0012).

v19 makes the CSO reporting-frequency question ("How frequently does your
organisation normally report?") a multi-select, so a CSO can pick more than one
reporting cadence. Existing single-value answers still display correctly.
"""
...
def activate(apps, schema_editor):
    ...
    Version.objects.create(
        schema=bundled,
        version_label=label,
        note="CSO reporting-frequency question is now multi-select.",
        is_active=True,
    )
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ("cso_mapping", "0012_activate_schema_v18"),
    ]
